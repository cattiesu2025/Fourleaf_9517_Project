...
--vocab-size (the hyperparameter you're most likely to tune) without ever
touching cv2.SIFT_create() again -- only clustering and histogram-building
re-run, which are fast.

Example usage:

  # Quick test on a small subset before running on the full class set.
  ./scripts/comp9517 train-sift \
      --train-csv data/metadata/train.csv \
      --test-csv data/metadata/val.csv \
      --num-classes 50 \
      --vocab-size 200 \
      --output-dir outputs/traditional/sift_bovw_svm_smoketest
  ...
  ./scripts/comp9517 train-sift \
      ...
      --vocab-size 300 \
      --cache-dir outputs/traditional/sift_bovw_svm/feature_cache \
      --output-dir outputs/traditional/sift_bovw_svm

  # vocab-size sweep with descriptor caching: the first run extracts and
  # caches raw SIFT descriptors (slow, the actual bottleneck of this
  # pipeline); every subsequent run with a different --vocab-size reuses the
  ...
  ./scripts/comp9517 train-sift \
      ...
      --cache-dir outputs/traditional/sift_bovw_svm/feature_cache \
      --vocab-size 200 \
      --output-dir outputs/traditional/sift_bovw_svm_vocab200
  ...
  ./scripts/comp9517 train-sift \
      ...
      --cache-dir outputs/traditional/sift_bovw_svm/feature_cache \
      --vocab-size 400 \
      --output-dir outputs/traditional/sift_bovw_svm_vocab400
  ...
  ./scripts/comp9517 train-sift \
      ...
      --vocab-size 300 \
      --cache-dir outputs/traditional/sift_bovw_svm/feature_cache \
      --load-vocab outputs/traditional/sift_bovw_svm/vocab.pkl \
      --load-model outputs/traditional/sift_bovw_svm/svm_model.pkl \
      --degradation gaussian_noise --severity 3 \
      --output-dir outputs/robustness/sift_bovw_svm/gaussian_noise/severity_3
"""
...
def extract_sift_descriptors(
    gray_img: np.ndarray, max_desc_per_image: int = 100, rng: np.random.RandomState = None
) -> np.ndarray:
    """
    ...
    descriptor count when clustering.
    """
...
def dataset_to_sift_descriptors(
    dataset: INatDataset,
...
# independent of --vocab-size, so vocabulary-size sweeps hit the cache; only
# --max-desc-per-image, --degradation/--severity, --num-classes, and the CSV
# file identity affect what gets cached.


def _cache_filename(
    csv_path: str, num_classes, max_desc_per_image, degradation_type, severity
) -> str:
...
def get_or_extract_sift_descriptors(
    dataset: INatDataset,
    csv_path: str,
    cache_dir,
    max_desc_per_image: int,
    ...
    cache_path = os.path.join(
        cache_dir,
        _cache_filename(csv_path, num_classes, max_desc_per_image, degradation_type, severity),
    )
    ...
    if os.path.exists(cache_path):
        ...
        if np.array_equal(cached["image_ids"], dataset.image_ids()):
            print(
                f"[cache] hit -- reusing cached SIFT descriptors for {len(cached['descriptors'])} images, "
...
def build_vocabulary(
    all_descriptors: np.ndarray, vocab_size: int, max_total_desc: int, seed: int = 9517
) -> MiniBatchKMeans:
    """
...
def histogram_from_descriptors(
    desc: np.ndarray, kmeans: MiniBatchKMeans, vocab_size: int
) -> np.ndarray:
...
def descriptors_to_bovw_features(
    per_image_desc, kmeans: MiniBatchKMeans, vocab_size: int
) -> np.ndarray:
...
def main():
    ...
    p.add_argument(
        "--test_csv",
        required=True,
        help="Use val.csv during development, test.csv only for the final run",
    )
    ...
    p.add_argument(
        ...
        "clustering + histogram + SVM -- useful when sweeping --vocab-size. "
